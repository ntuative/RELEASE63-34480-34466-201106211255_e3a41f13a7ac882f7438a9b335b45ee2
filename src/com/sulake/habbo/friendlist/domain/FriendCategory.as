package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         if(this.var_927 >= this.getPageCount())
         {
            this.var_927 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_238.length / const_406);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_927 * const_406;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_927 + 1) * const_406,this.var_238.length);
      }
