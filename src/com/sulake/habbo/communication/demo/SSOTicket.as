package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_348:String = "failure";
      ...
      private var var_1484:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         {
            this.var_1484.init();
            this.var_1484.addEventListener(HabboWeb.const_295,this.requestClientURL);
         }
         else
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1484.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_1814 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_1799 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2473 = Boolean(facebookData[1]);
               }
               if(this.var_1814.length > 0)
               {
