package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_2449:PseudoRandom;
      ...
      private const const_2051:String = "fuselogin";
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_628 = param1;
            this.var_64.initConnection(HabboConnectionType.const_241);
         }
      }
      ...
      private function onRoomSessionManagerReady(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_16.events.addEventListener(RoomSessionEvent.const_81,this.onRoomSessionEnded);
         this.checkRequirements();
      }
      ...
      {
         if(this.var_64 && this.var_147 && this.var_16 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         var _loc1_:Boolean = this.var_147.getBoolean("client.hotel_view.show_on_startup",true);
         this.var_242 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         var _loc2_:String = this.var_147.getKey("client.hotel_view.image");
         var _loc3_:String = this.var_147.getKey("image.library.url");
         if(_loc2_ != null && _loc3_ != null)
         {
            this.var_242.loadHotelViewImage(_loc3_ + _loc2_);
         }
         var _loc4_:* = this.var_147.getKey("use.sso.ticket") == "1";
         this.var_628 = this.var_147.getKey("sso.ticket",null);
         this.var_1799 = this.var_147.getKey("flash.client.url","");
         this.var_2450 = this.var_147.getKey("external.variables.txt","");
         if(this.var_242 == null)
         {
            this.var_242 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         }
         if(_loc4_)
         {
            this.var_64.mode = HabboConnectionType.const_97;
            if(this.var_628)
            {
               this.var_64.initConnection(HabboConnectionType.const_241);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc5_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_624,this.onInitConnection);
            }
            else
            {
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_64.mode = HabboConnectionType.const_97;
            if(this.var_147.keyExists("local.environment"))
            {
               this.var_64.mode = HabboConnectionType.const_1328;
            }
         }
         this.var_64.initConnection(HabboConnectionType.const_241);
      }
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2449 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_64.mode)
         {
            case HabboConnectionType.const_97:
               _loc6_ = "";
               _loc6_ = this.var_147.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_147.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1475 = _loc4_;
               this.var_242.loadBannerImage(_loc7_ + "?token=" + this.var_1475,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1328:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1899:
               this.var_962 = this.generateRandomHexString(30);
               this.var_523 = new DiffieHellman(new BigInteger(this.var_147.getKey("connection.development.prime"),16),new BigInteger(this.var_147.getKey("connection.development.generator"),16));
               this.var_523.init(this.var_962);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2449);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         connection.send(new VersionCheckMessageComposer(401,this.var_1799,this.var_2450));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_2051,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_628 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_628);
            _loc2_.send(_loc4_);
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(param1.type == Event.CLOSE && !this.var_2448)
         {
            _loc2_ = this.var_147.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2448 = true;
         var _loc2_:String = this.var_147.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_147.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1475);
