package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1071:int = 3;
      
      private var var_478:AssetLoaderStruct;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_478 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_1069 = new Timer(this.const_2340,1);
         this.var_1069.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_478)
         {
            this.var_478.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_478.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_478)
         {
            this.var_478.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_478.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1071 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1314);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1071);
            if(this.var_1314.indexOf("?") > 0)
            {
               _loc2_ = this.var_1314 + "&retry=" + this.var_1071;
            }
            else
            {
               _loc2_ = this.var_1314 + "?retry=" + this.var_1071;
            ...
            this.var_478 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2038)
         {
            this.var_1315.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         {
            delete this.var_1070[_loc3_];
         }
         while(_loc6_ < this.var_844.length)
         {
            _loc5_ = this.var_844[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_844.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1069.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_845.length > 0 && this.var_844.length < this.const_1911)
         {
            _loc1_ = this.var_845[0];
            _loc1_.startDownloading();
            this.var_844.push(this.var_845.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_845.indexOf(param1) == -1 && this.var_844.indexOf(param1) == -1)
         {
