package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private const const_2342:int = 13421772;
      
      private const const_2341:int = 16777215;
      
      private const const_381:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_732 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_2048 = Math.max(this.var_2048,_loc6_.colorLayerIndex);
            }
         }
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_732 != null && !this.var_2049)
            {
               _loc3_ = this.var_732;
            }
            else
            ...
            if(this.var_847)
            {
               switch(this.var_847.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_45:
                     _loc2_.style = HabboIconType.const_733;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1614.color = this.const_2342;
         }
         else
         {
            this.var_1614.color = this.const_2341;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_381[_loc6_] + "_" + FigureData.const_163;
               _loc4_ = this.var_479.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_381.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_381[_loc6_] + "_" + FigureData.const_163;
                  _loc4_ = this.var_479.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_683 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2050)
               {
                  ...
                  this.var_2050 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_2049 && _loc2_.colorLayerIndex > 0)
               {
