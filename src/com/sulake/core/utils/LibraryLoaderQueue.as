package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1911:int = 4;
      ...
      {
         return this.var_487.length + this.var_306.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_306.length < const_1911 && this.var_487.length > 0)
            {
               _loc1_ = this.var_487.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_487.indexOf(loader);
            if(index > -1)
            {
               this.var_487.splice(index,1);
            }
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_487)
            {
               if(param2)
               {
